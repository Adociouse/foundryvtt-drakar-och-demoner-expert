...
**Klientspråk:** svenska (`lang/sv.json`) och engelska (`lang/en.json`) för allt UI-chrome (fältetiketter, knappar, notiser, chattkort). `regler`-kompendiets 12 regelsidor är dessutom fullt översatta och växlar automatiskt med klientens språkval (en `JournalEntryPage` per språk i samma dokument — se "Kända begränsningar"). Övrigt kompendieinnehåll (vapen, besvärjelser, raser, yrken, monster, `sl-regler`/`journaler`) är på svenska oavsett klientspråk.
...
| Kompendier: 13 raser, 39 yrken, 393 vapen/utrustning, 475 besvärjelser (14 magiskolor), 242 monster | Klar (fortsatt luckor i bildtäckning, samt i bestiary-täckningen mot källböckerna och beskrivningstexternas kvalitet — se nedan). Besvärjelsekatalogens katalogkomplettering mot Formelboken är HELT KLAR; Kaos Väktares demonologitillägg (3 yrken, Portalmagi som 14:e skola) tillagt 2026-09-02 |
...
- **Vapensortimentet täcker nu alla 49 poster i `DODE.weaponGroups`-tabellen (2026-09-08), plus Bola/Lasso/Oxpiska/Silverdolk som specialvapen — och ytterligare 18 vapen ur Krigarens Handboks (KH) eget vapenkapitel — 71 vapen totalt.** Sourcat med RP&gt;SL&gt;SB&gt;KH&gt;REG-precedensen: 19 av de 29 `weaponGroups`-vapnen kommer från Spelarboken (där REG antingen saknar vapnet helt eller ger avvikande skadetärningar mot samma bokprecedensregel), 9 från REG (SB täcker inte vapnet alls), och Tornerlans från Krigarens Handbok (varken REG eller SB har den). ⚠ KH kör dessutom ett HELT EGET, uttryckligen valfritt vapenkapitel (KH s.46-51) med skadan delad i kroppspoäng (KP) och smärtpoäng (SP) i stället för REG:s/SB:s enda skadetal — inkompatibelt med kompendiets schema, som bara har ETT `damage`-fält. **Två olika lösningar beroende på fallet:** vapen där KH:s KP-kolumn är ett vanligt skadetal (18 st: Testikeldolk, Stilett, Falchion, Långsvärd, Sabel, Stickvärja, Värja, Stridsklubba, Flamberge, Tvåhandsgissel, Tvåhandshammare, Jaktspjut, Lätt lans, Bondeslaga, Niosvansad katt, Stridsgissel med spik) fick KP-talet som sitt vanliga, rullbara `damage`-värde, med SP-talet sparat som en förklarande not i beskrivningen (SL kan lägga till det manuellt om hen kör det fulla KP/SP-systemet). Vapen där KP är obefintligt eller inte meningsfullt (Tornerlans, Träningssvärd, Knogjärn) fick `"Spec (...)"`-behandling i stället, samma mönster som redan används för Bola/Lasso/Blåsrör. **Två poster (Trädstam liten/stor) byggdes medvetet INTE** — improviserade klubbor vars pris/vikt uttryckligen "varierar beroende på träslag" i källan, ingen fast statrad möjlig utan en egen materialvalsmekanism. Hela läsningen (7 tabeller + kringtext) sparad som ett kurerat extrakt, `docs/extracts/DODE_KH_VAPEN_KP_SP.md` (Roll20-projektet). ⚠ Samma vapenpass hittade och rättade en genuin, tidigare osynlig bugg: 66 poster (i huvudsak rustningsdelar och sköldar) hade `source.book: "spelarboken"` — en ogiltig nyckel (rätt nyckel är `"sb"`) som `CONFIG.DODE.books`s enum tyst nollställde till tomt vid varje dokumentladdning, så källhänvisningen visade "okänd källa" i UI:t trots att `_source`-filen såg korrekt ut.
...
- **Minibesvärjelser visas på arket men går inte att kasta än.** Katalogen (44 poster), den skolhärledda åtkomsten (alla skolor rollpersonen har färdighet i, inte bara högst FV) och arkets minimagi-lista är alla byggda — men ingen kastknapp finns än, så inget spenderar den 1 PSY:n eller postar ett chattkort. De 12 allmänna minibesvärjelserna är dessutom just nu onåbara oavsett rollpersonens skolor, en riktig bugg i härledningslogiken, inte avsiktligt. Se `docs/DESIGN_DECISIONS.md` backlog 25.
...
- **Bara `regler`-kompendiets 12 regelsidor är översatta till engelska** — allt annat kompendieinnehåll (vapen, besvärjelser, raser, yrken, monster, `sl-regler`, `journaler`) är fortfarande enbart svenska, oavsett klientspråk. Mekanismen (en engelsk `JournalEntryPage` per svensk sida, filtrerad mot `game.i18n.lang` med automatisk fallback till svenska om ingen översättning finns) är byggd för att generaliseras till fler kompendier om det efterfrågas. Rollpersonsguidens egna förklaringstexter (hintar, stegöversikten, granskningssidan) är däremot fullt översatta — se nästa punkt.
