...
> Framtagen 2026-08-24 på Projektets begäran: *"the selection of spells is working as it should in wizard. Then we need to analyze which spells could be used in combat or not and do a plan for how they will be mechanically implemented."*
>
...
| **Illusionism** | 23+5=28 | ✅ Klar | 2026-09-03 | 2 räddningsslag (Färglöshet/int, Skrik/psy — Skrik ger även PSY-förlust vid misslyckande, INTE satt som instantEffect eftersom `kind:"damage"` representerar KP-skada, inte en PSY-resursförlust), 11 till `battleRelevant:true`-only (distraktion/villospår/dubbelgångare/tystnadszon/en illusorisk stridande med egna grundegenskaper m.fl.). 12 rörda inte alls. **Genuint arkitekturfynd, inte en bugg:** ett försök att flagga minimagin "Individuell distraktion" som `battleRelevant` (den är uttryckligen en mindre version av den nu kurerade Distraktion) skrevs till källfilen men **försvann tyst vid packning** — `item-minibesvarjelse.mjs` är ett medvetet SEPARAT, minimalt DataModel (Projektets beslut 2026-07-28, dokumenterat i filens egen header) utan `battleRelevant`/`instantEffect`/`statusEffect`/några av de mekaniska fälten alls, eftersom minimagi kategoriskt saknar CL-slag/effektgrad. **Ingen minibesvärjelse kan alltså någonsin mekaniskt kureras med dagens schema** — värt att veta innan nästa skola frestas göra samma försök. Ej en avvikelse att rätta, bara en gräns att känna till. |
| **Mentalism** | 63+2=65 | ✅ Klar | 2026-09-03 | Störst skola — domineras av fysiska/sensoriska självbuffar (flyg/ABS/STY/SMI/andning m.fl.), inte direktskade-/statusbesvärjelser. 1 skadebesvärjelse (Elchock, beröringsbaserad, `@{E}d4` blixt, liverullad och verifierad). 1 statuseffekt (Snubbla → `prone`, ny bekräftad kärn-id från kreaturstyp-passet). 33 till `battleRelevant:true`-only (buffar/debuffar utan modifierarfält, opponerad kontroll/teleportering/telekinesi, reträttverktyg). 1 redan kurerad sedan tidigare (Fjällpansarhud). **Genuin schemalucka, samma klass två gånger:** Motståndskraft (flat eld/kyla-reduktion) och Syraskydd (full syraimmunitet) är EXAKT de bokmönster `resistances[]` redan uttrycker — men `resistances[]` lever statiskt på aktörens schema, ingen motor kopplar idag en besvärjelsekastning till att TILLFÄLLIGT lägga till en sådan post. Lämnade omekaniserade på den punkten. Magisk syn (S13) saknar källtext helt i boken (redan flaggat i beskrivningen) — inget att klassificera. **Spiritisms "Mental projektil" hänvisar till "Mental Attack"** — ingen sådan namngiven besvärjelse hittades i Mentalism-katalogen under denna genomläsning; sannolikt aldrig transkriberad, eller finns under ett annat namn. Kvarstår omekaniserad, inte jagad vidare denna omgång. 28 rörda inte alls (ren sensorisk/social/detektionsutility). |
| **Nekromanti** | 46+8=54 | ✅ Klar | 2026-09-03 | **SISTA skolan — hela kureringsomgången klar.** 1 PSY-skadebesvärjelse (Livsuttömning, `@E*2`, `resource:"psy"`, liverullad och verifierad end-to-end: 10→4 PSY vid E3). 6 statuseffekter (Dövhet→deaf, Förstumma→silence, Chock→stun, Voodooritual→paralysis, Förstening→unconscious — plus redan kurerade Blindhet→blind, Paralysering→paralysis). 26 till `battleRelevant:true`-only (naturliga vapenbuffar, odöd-skapande stridstjänare, permanenta lem-/sinnesförlamningar, villkorade skräcktabell-utlösningar m.fl.). **Kraftstöld flaggad som ny schemalucka:** PSY-dränering vars tak (max 1/10 av MÅLETS eget PSY) inte går att uttrycka — `instantEffect.formula` kan bara referera kastarens effektgrad, inte målets aktuella resurs. **Kontrollera lägre/högre odöd riktar sig mot en BLANDNING av creatureType-kategorier** (t.ex. gasthäst=spirit bland annars undead-corporeal-mål) — `targetRestriction` stödjer bara ett värde, lämnad osatt hellre än att felaktigt utesluta giltiga mål. "Skapa en stridsduglig odöd tjänare"-ritualer (Animera död/tupilak, Massa, Zombiekopia, Spökskepp) fick `battleRelevant:true`; "återuppliva en specifik, namngiven person SOM SJÄLV VÄLJER" (Frammana dödsriddare/gast, Spökdråp) lämnade omekaniserade, konsekvent med hur de redan behandlades i Demonologi/Spiritism. 19 rörda inte alls. |
| **Portalmagi** | 7+0=7 | ✅ Klar | 2026-09-03 | Ny skola (Kaos Väktare, 2026-09-02). Hela skolan är rese-/portalinfrastruktur, inte stridsmagi. 2 `battleRelevant:true`-only: Låsa portal (försvarsvakt, indirekt 1T3/E-skada mot en varelse som tvingar sig igenom — en passiv fälleeffekt, inte en direkt cast-mot-mål, därför inget instantEffect satt) och Förgöra portal (objektförstörelse, kan sabotera en fiendes reträttväg). 5 rörda inte alls. |
| **Röstmagi** | 18+0=18 | ✅ Klar | 2026-09-03 | Röstbaserad krigsmagi — starkt stridsanpassad skola. 2 skadebesvärjelser (Röstprojektil `1d6+@E`, Krossa odöd `@{E}d10`, båda damageType `physical`, Krossa odöd bara mot odöda med kropp — målrestriktion ej uttryckbar i schemat). 1 statuseffekt (Halt→`paralysis`). 9 till `battleRelevant:true`-only (STY-buff, dövhet, knuffa omkull, kommando/tvångslydnad, objektförstörelse, två automatiska Skräcktabell-utlösningar utan separat måls-slag). 1 redan kurerad (Stridsrop). **Ljudstyrka flaggad som schemalucka:** eskalerande per-SR-skada (1,2,3...) som varken `instantEffect` eller dagens fastbelopps-periodeffektmotor fångar exakt. 5 rörda inte alls. |
| **Spiritism** | 24+0=24 | ✅ Klar | 2026-09-03 | 1 statuseffekt (Själafälla→`unconscious`, offrets kropp i koma). 13 till `battleRelevant:true`-only. **Andeslag och Själaförvittring är riktiga skadebesvärjelser men mot PSY, inte KP** — `instantEffect.kind:"damage"` representerar KP-skada, så INGENDERA fick instantEffect satt (samma princip som Illusionisms Skrik). Mental projektil hänvisar uttryckligen till Mentalisms "Mental Attack" (ej kurerad än) — lämnad utan instantEffect tills den skolan görs och den verkliga formeln kan speglas. 10 rörda inte alls. |
...
| **Symbolism** | 27+4=31 | ✅ Klar | 2026-09-03 | Fällsymbol-magi ("de som ser symbolen ...") — starkt stridsanpassad, triggar på synintryck. 1 skadebesvärjelse (Försvar `@{E}d3`, physical, liverullad och verifierad), 1 helbesvärjelse (Läka `@E`, helar ALLA som ser symbolen — targetMode `area`), 3 statuseffekter (Paralysering→paralysis, Blindhet→blind, Epilepsi→unconscious, Plåga→paralysis [handlingsoförmögen av smärta, funktionellt samma stridsutfall]). 15 till `battleRelevant:true`-only (CL-debuff, zonspärr, lockelse-fälla, fördröjd triggermekanism (Kraft), anti-teleport, elementar-/andeförvisning, stridsmods-buff, automatisk Skräcktabell-utlösning, hypnos, charm, en frammanad stridsduglig stenstaty). 7 rörda inte alls. |

**Arbetsordning:** skolor väljs en i taget (se ACTIVE_TASK.md för senaste). Poster-antal ovan är från en live-räkning 2026-09-03 (`node`-skript mot `packs/besvarjelser/_source`) — räkna om om det känns fel, `_source` är alltid facit.

**✅ HELA kureringsomgången klar, 2026-09-03 — alla 15 skol-motsvarigheter (13 spelbara skolor + Allmänna + Portalmagi) genomlästa mot det fulla mekaniska schemat.** Startad efter Projektets direkta fråga "have all new spells... mechanical effects for battle?" (svaret var nej), avslutad samma dag efter en lång, sammanhängande session. Ingen skola hoppades över — Alkemi är enda "N/A" (inget innehåll byggt i den skolan alls). Detta är INTE slutet på besvärjelsemekanik-arbetet — se de genuina schemaluckor som hittades UNDER passet, nu i `docs/DESIGN_DECISIONS.md`s backlog (84 proportionell/vapenkopplad resistances[], 90 spellEffect cast→apply-koppling, m.fl.) — men den SYSTEMATISKA genomläsningen av varje besvärjelse mot schemat är komplett. En framtida session som hittar en okurerad besvärjelse ska alltså anta att den GENOMLÄSTS och medvetet lämnats vid sina defaultvärden, inte att den missats.

**Retroaktiv fix-omgång, 2026-09-03 (samma dag, direkt efter de nio skolorna ovan)** — Tre direkta uppföljningsfrågor ställdes efter Symbolism: (1) hur är Krossa odöds "bara mot odöda"-restriktion egentligen wired in, (2) okända statuseffekt-id:n ska avgöras direkt eller läggas i backlog, aldrig bara tyst utelämnas, (3) PSY-skada måste kopplas in i stridsmotorn. Svar/åtgärder:
- **PSY-skada är nu en riktig, byggd mekanik.** Nytt fält `instantEffect.resource: "hp"|"psy"` (`item-besvarjelse.mjs`), motorstöd i BÅDA faserna av `scripts/rolls/spell.mjs` (`resolveSpellCast`/`applySpellResult`), och `templates/chat/spell-card.hbs` visar nu "PSY" i stället för "KP" när relevant. Tillämpat på **Andeslag** (`@E`, Spiritism), **Själaförvittring** (`1d3+@E`, Spiritism) och **Skrik** (`@E`, Illusionism) — alla tre var tidigare `battleRelevant:true`-only just för att detta fält saknades. Liveverifierat end-to-end mot en riktig `character`-aktör (10→7 PSY, exakt matchande `appliedAmount:3`).
- **Genuin, tidigare okänd lucka hittad UNDER samma verifiering: NPC-aktörer har INGEN deplomerbar PSY-resurs alls.** `actor-npc.mjs`s `attributes.psy` är bara `{value}` — en platt attributpoäng, ingen `resources.psy.value/max`-motsvarighet som `actor-character.mjs` har. En PSY-skadebesvärjelse riktad mot en NPC har alltså i dag ingenstans att skriva — samma lucka gäller symmetriskt för kastarens EGEN PSY-kostnad om en NPC kastar en besvärjelse (redan existerande kod, inte ny av detta pass). Ny backlogpost 92 i DESIGN_DECISIONS.md.
- **Krossa odöds "bara mot odöda"-restriktion är INTE mekaniskt genomdriven.** `resolveSpellCast` applicerar `instantEffect`-skada på VILKET mål som helst som skickas in, oavsett varelsetyp — ingen kod läser `target.type`/monstertagg för att blockera felaktig målsättning. Samma "systemet möjliggör, spelaren/SL avgör"-princip som `attackUnprepared` redan bygger på — inte ett fel, men värt att känna till innan man antar att systemet skyddar mot felkastning. Ny backlogpost 93.
- **Sju statuseffekt-id:n avgjorda direkt mot en LIVE-hämtad `CONFIG.statusEffects`-lista** (inte gissade): `Bedövning`/`Bedövad` → `deaf`, `Tryckvåg` → `prone`, `Tystnad` → `silence`, `Skrän`/`Förvirra` → `stun`, `Flykt` → `fear` — alla bekräftat existerande Foundry-kärnstatusar, tidigare felaktigt utelämnade av försiktighet. **Två kvarstår utan matchning** (`Raseri`/`Massraseri`s tvingade bärsärkagång, `Grodförvandling`s permanenta formförvandling) — inget kärn-id passar ("berserk"/"polymorph" finns inte i listan). Ny backlogpost 94 (custom status som `armObrukbar`/`handUpptagen`, eller lämna narrativt).
- **Minimagi-i-strid** — Illusionismradens fynd (inga mekaniska fält alls på `minibesvarjelse`) läggs upp som en egen analysfråga, ny backlogpost 95: BÖR minimagi någonsin ha stridsmekanik, och om ja, vilket minimalt schematillägg (utan att dra in CL-slag/effektgrad som typen medvetet saknar) skulle krävas?

Full bekräftad `CONFIG.statusEffects`-lista (hämtad live 2026-09-03, denna Foundry-version): `dead, unconscious, sleep, stun, prone, restrain, paralysis, fly, blind, deaf, silence, fear, burning, frozen, shock, corrode, bleeding, disease, poison, curse, regen, degen, hover, burrow, upgrade, downgrade, invisible, target, eye, bless, fireShield, coldShield, magicShield, holyShield, armObrukbar, handUpptagen`. Slå upp HÄR innan du gissar eller utelämnar en statusEffect-id i framtida skolor.
