...
| Hjältedådstabell (HH s.6-7) nu wired in i nivå-steget | **Done (2026-08-02)** | `DODE.hjaltedadTable`/`hjaltedadRollCount` fanns redan i `config.mjs` men anropades aldrig — de fyra nivåkorten visade därför identisk "125 BP" (Johans fynd #1), trots att bara `vanlig` faktiskt ska sakna bonusrullning. Ny `#onRollHjaltedad`: för `slumpens-hjalte`/`sann-hjalte`/`gudafodd` slås `1T6` (via ny `DODE.swedishDiceToRoll`-regex, "1T6"→"1d6") för antal `1T20`-slag mot `hjaltedadTable`, som summerar `bonusBP`/`bonusHP` (dels flata tal, dels tärningsformler som `"1T10+10"`, utvärderade som riktiga `Roll`). Tabellens `notes`-kolumn (t.ex. "+10 Startkapital", "Magiskt vapen", "15 hjältepoäng") visas som text åt SL att tillämpa för hand — se backlogpost 58, inte byggt denna omgång. Nya schemafält `bp.bonusHjaltedad`/`hp.bonusHjaltedad` i `actor-character.mjs`, adderade i `bp.start`/`hp.max`. Liveverifierat: Sann hjälte-rullning gav 73 bonus-BP (start 198 = 125+73) + 26 bonus-HP (max 34), sparat korrekt på en skapad testaktör. |
