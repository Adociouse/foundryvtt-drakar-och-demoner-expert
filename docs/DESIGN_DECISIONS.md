...
| ⚠ EP delas INTE ut i strid | **Bugg, ej åtgärdad (2026-07-29)** | Upptäckt i den fullständiga stridssimuleringen. `resolveAttack` använder sin egen `classifiedRoll` i stället för `rollFV`, så **sömnklockans EP-utdelning triggas aldrig i strid** — en rollperson kan slåss en hel dag och lära sig ingenting. Simuleringen delade ut EP manuellt för att visa vad som borde hänt (Svärd 1, Yxor 1). ⚠ Dessutom: **en lyckad PARERING ger heller ingen EP**, trots att RP s.63 ger EP för lyckad användning av en färdighet — och en parering är en användning av Sköld. Båda behöver åtgärdas i samma veva. |
