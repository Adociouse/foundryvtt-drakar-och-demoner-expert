...
24. **Distinct per-spell icons (165).** The 168 spells ported 2026-07-27 all display their magic school's sigil rather than individual art (see backlog 14b for why that was chosen over a 165-image generation run). Not a placeholder, but per-spell icons would be better — `docs/dev/ART_STYLE.md`'s spell/arcane-symbol template and the slug already written into each item's `img` path make this a mechanical batch whenever it's worth the time.
