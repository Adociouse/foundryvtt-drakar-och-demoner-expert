...
- **The weapon roster now covers all 49 entries in the `DODE.weaponGroups` table (2026-09-08), plus Bola/Lasso/Oxpiska/Silverdolk as special weapons — 53 weapons total.** Sourced per the RP&gt;SL&gt;SB&gt;KH&gt;REG precedence rule: 19 of the 29 newly-built weapons come from Spelarboken (SB) — either because REG doesn't cover the weapon at all, or gives conflicting damage dice against that same precedence rule — 9 from REG (SB doesn't cover them), and the last one — **Tornerlans** (jousting lance) — from Krigarens Handbok (neither REG nor SB has it, so no two books compete over the same entry there). ⚠ Tornerlans' own source (KH p.50) runs a separate, optional damage system (body points/pain points split into two columns) that the rest of the compendium doesn't implement — its `damage` field is set to `"Spec (...)"` with an explanatory note, the same pattern already used for Bola/Lasso/Blåsrör; the GM applies the pain-point effect manually. ⚠ The same pass found and fixed a genuine, previously invisible bug: 66 entries (mostly armor pieces and shields) had `source.book: "spelarboken"` — an invalid key (the correct one is `"sb"`) that `CONFIG.DODE.books`'s enum silently reset to blank on every document load, so the source citation showed "unknown source" in the UI even though the `_source` file looked correct.
