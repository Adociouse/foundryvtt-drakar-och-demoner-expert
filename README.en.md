...
| Battlemap movement & encumbrance — a real load table (SPB p.44) that actually affects movement/agility/skill checks, DoD's own movement tiers on the drag ruler (can still attack / full move / run / sprint), 3D-elevation-aware melee reach, terrain via Foundry's own map regions | Done, see details below |
