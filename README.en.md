...
**Client language:** Swedish (`lang/sv.json`) and English (`lang/en.json`) for all UI chrome (field labels, buttons, notifications, chat cards). The `regler` (rules) compendium's 12 rules pages are also fully translated and switch automatically with the client's language (one `JournalEntryPage` per language in the same document — see "Known limitations").
...
| Battlemap movement & encumbrance — a real load table (SPB p.44) that actually affects movement/agility/skill checks, DoD's own movement tiers on the drag ruler (can still attack / full move / run / sprint), 3D-elevation-aware melee reach, terrain via Foundry's own map regions, rider↔mount linking | Done, see details below |
...
| Hero-points spending window (HH p.20/46-48: special ability, heroic ability, base attribute, Raise CL) | Done, own `ApplicationV2` view (`scripts/apps/hero-points.mjs`) — see "Known limitations" for Raise CL's bookkeeping/rule-application split |
...
### Battlemap movement & encumbrance

The load table (`system.encumbrance`, Spelarboken p.44) sums carried weight (worn armor counts at half weight, clothing not at all) and gives it a real consequence: past a threshold, movement, agility, and CL on all agility-based skills drop together (floored at 1), and an "Encumbered" status icon appears on the token automatically. In combat, a custom ruler (`CONFIG.Token.rulerClass`) colors the drag path in DoD's own tiers — can still attack (≤½×), full move (1×), Run (2×), Sprint (3×) — by reading Foundry's own `movementHistory`, reset per combat round instead of Foundry's default per-turn reset. Purely visual/analytical — no hard block; the GM judges. Elevation differences (e.g. a balcony) are now measured in true 3D, so a melee attack across levels is automatically rejected if the distance exceeds the weapon's reach. Terrain (swamps, deep snow) is handled with Foundry's built-in map regions — no bespoke system. The sheet also shows a "Can't swim" row when armor (regardless of weight, Rollpersonen p.55) or encumbrance (Spelarboken p.44) blocks swimming — informational only, no hard block. Monsters/NPCs get their own per-movement-type budget (walk/fly/swim/…) parsed live from their source text ("F30/L26"), so the ruler colors their drag just as correctly as a character's. A rider can be mounted onto a mount (`game.dode.mountToken()`) and then automatically follows it whenever the mount moves on the map — see the packaged "Tung stridshäst" (Heavy war horse) NPC and the "Ridning & riddjur" (Riding & Mounts) rules page (Krigarens Handbok p.27-29: mounted-combat skill checks, charge attacks, ranged weapons from horseback, being thrown from the saddle, mounted maneuvers).
...
| `vapen-utrustning` | 393 entries: 71 weapons (incl. Silverdolk, `material:"silver"`, 2026-09-03 — the full `DODE.weaponGroups` table fully transcribed 2026-09-08, plus 18 weapons from Krigarens Handbok's own optional weapon chapter, see Known limitations), 45 armor pieces (per body part), 277 general equipment (incl. 6 ammunition entries — Pilar/Silverpilar/Armborstpilar/Silverbultar/Slungstenar/Silverkulor, `category:"ammunition"`, whose `material` field drives the combat resistance check — "the bow is just a bow, the arrow is what's silver") — purchasable in the wizard's equipment step. Weight is in kg (Spelarboken p.44/p.12, migrated from the Magi rulebook's BEP table 2026-09-06 — see Known limitations) |
...
- **The item-weight BEP→kg migration (×3) doesn't reproduce Spelarboken's own kg-priced equipment lists item-for-item.** The conversion factor (1 BEP = 3 kg) is exactly what Spelarboken's own errata (p.12) specifies, but the Magi rulebook's and Spelarboken's equipment lists appear to have been weighted independently for the same objects — a 19-item spot check found only 1 match. See `docs/DESIGN_DECISIONS.md` backlog 117 for a possible future item-by-item reconciliation.
...
- **The weapon roster now covers all 49 entries in the `DODE.weaponGroups` table (2026-09-08), plus Bola/Lasso/Oxpiska/Silverdolk as special weapons — and 18 more weapons from Krigarens Handbok's (KH) own weapon chapter — 71 weapons total.** Sourced per the RP&gt;SL&gt;SB&gt;KH&gt;REG precedence rule: 19 of the 29 `weaponGroups` weapons come from Spelarboken (either REG doesn't cover the weapon at all, or gives conflicting damage dice against that same precedence rule), 9 from REG (SB doesn't cover them), and Tornerlans from Krigarens Handbok (neither REG nor SB has it). ⚠ KH also runs a whole separate, explicitly optional weapon chapter (KH p.46-51) that splits damage into body points (KP) and pain points (SP) instead of REG's/SB's single damage number — incompatible with the compendium's schema, which only has one `damage` field. **Two different handling approaches depending on the case:** weapons where KH's KP column is a normal damage value (18 of them: Testikeldolk, Stilett, Falchion, Långsvärd, Sabel, Stickvärja, Värja, Stridsklubba, Flamberge, Tvåhandsgissel, Tvåhandshammare, Jaktspjut, Lätt lans, Bondeslaga, Niosvansad katt, Stridsgissel med spik) got the KP number as their normal, rollable `damage` value, with the SP number saved as an explanatory note in the description (the GM can add it manually if running the full KP/SP system). Weapons where KP is absent or meaningless (Tornerlans, Träningssvärd, Knogjärn) got `"Spec (...)"` treatment instead, the same pattern already used for Bola/Lasso/Blåsrör. **Two entries (Trädstam liten/stor) were deliberately NOT built** — improvised clubs whose price/weight explicitly "varies by wood type" in the source, no fixed stat row possible without a real materials-choice mechanism. The whole read (7 tables plus surrounding text) was saved as a curated extract, `docs/extracts/DODE_KH_VAPEN_KP_SP.md` (Roll20 project). ⚠ The same pass found and fixed a genuine, previously invisible bug: 66 entries (mostly armor pieces and shields) had `source.book: "spelarboken"` — an invalid key (the correct one is `"sb"`) that `CONFIG.DODE.books`'s enum silently reset to blank on every document load, so the source citation showed "unknown source" in the UI even though the `_source` file looked correct.
- **The bestiary covers 242 creatures** from Monsterboken 1, Monsterboken 2, Monsterboxen II and the Svartfolk supplement — all four COMPLETE, including Svartfolk's named NPCs and its ready-to-drop svartfolk archetypes. ⚠ Coverage against the full source material is **not** established, however: besides Monsterboxen IV (56 entries, none built), two further books containing creature statblocks have not yet been audited — *Monster och Man i Ereb Altor* and *Drakar*.
...
- **Minibesvärjelser (minor spells) show on the sheet but can't be cast yet.** The 44-entry catalogue, school-derived access (every school the character has skill in, not just highest-FV), and the sheet's minimagi list are all built — but there's no cast button yet, so nothing actually spends the 1 PSY or posts a chat card. The 12 general (`allman`) minor spells are additionally unreachable right now regardless of the character's schools, a real bug in the access-derivation logic, not by design. See `docs/DESIGN_DECISIONS.md` backlog 25.
...
- **Svartfolk (Dark Folk) isn't available as a playable race option in the character wizard.** The Svartfolk supplement's NPC/monster content (named NPCs, archetypes) is built and included in the bestiary above — but a playable Svartfolk base race with its own lineages (the same shape as Elf/elf lineages) hasn't been started.
- **Only the `regler` compendium's 12 rules pages are translated to English** — all other compendium content (weapons, spells, races, professions, monsters, `sl-regler`, `journaler`) remains Swedish-only regardless of client language. The mechanism (one English `JournalEntryPage` per Swedish page, filtered against `game.i18n.lang` with an automatic fallback to Swedish if no translation exists) is built to generalize to more compendiums if that's ever requested. The character wizard's own explanatory text (hints, the step overview, the review page) is fully translated, though — see the next point.
- **Skill names, ability names, and the wizard's static step labels (e.g. "Kön", "Nivå", "Ras") aren't localized** — they're hardcoded in `scripts/apps/character-wizard.mjs` (STEP_LABELS, gender/tier options) and `scripts/helpers/config.mjs` (`DODE.skills`) respectively, separate from the template-driven text. An English-speaking player sees an English UI shell around a handful of Swedish nouns.
- **The hero-points "Raise CL" branch (HH p.20/46-48) only automates the bookkeeping.** The button deducts 1 hero point from the pool, but applying the actual rule (which outcome tier it shifts to) is done manually by the GM — see `docs/dev/SPECIALANFALL_SL_GUIDE.md`. The other three branches (acquire special ability, acquire heroic ability, improve base attribute) are fully automated.
