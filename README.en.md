...
- **The weapon roster covers 51 of 52 entries in the `DODE.weaponGroups` table** (2026-09-08) — only **Tornerlans** (jousting lance) is missing, since no curated source document has turned up a numeric stat row for it yet (only flavor-text description). Sourced per the RP&gt;SL&gt;SB&gt;KH&gt;REG precedence rule: 19 of the 28 newly-built weapons come from Spelarboken (SB) — either because REG doesn't cover the weapon at all, or gives conflicting damage dice against that same precedence rule — the rest from REG where SB doesn't cover the weapon. ⚠ The same pass found and fixed a genuine, previously invisible bug: 66 entries (mostly armor pieces and shields) had `source.book: "spelarboken"` — an invalid key (the correct one is `"sb"`) that `CONFIG.DODE.books`'s enum silently reset to blank on every document load, so the source citation showed "unknown source" in the UI even though the `_source` file looked correct.
